"""Stored global-job search and candidate recommendations."""
...
repository = GlobalJobRepository()


def _response(job: GlobalJob, scores: dict[str, float] | None = None) -> JobResponse:
    values = {
        "id": job.id,
        "source": job.source,
    ...
    return JobResponse(**values, **(scores or {}))


def _page(items: list[JobResponse], page: int, page_size: int, total: int) -> JobPage:
    return JobPage(
        items=items,
        page=page,
        page_size=page_size,
...
@router.get("", response_model=JobPage, tags=["jobs"])
async def search_jobs(
    ...
    employment_type: Annotated[str | None, Query(max_length=100)] = None,
    ...
    return _page([_response(row) for row in rows], page, page_size, total)


@router.get("/recommended", response_model=JobPage, tags=["jobs"])
async def recommended_jobs(
    ...
    candidate_profile = await session.scalar(
        ...
        .where(CandidateProfile.owner_profile_id == profile.id)
    ...
    rows, _ = await repository.search(
        session,
        role_families=tuple(ROLE_COMPATIBILITY.get(candidate_role, (candidate_role,))),
        page=1,
        page_size=500,
    )
    ranked = [(job, rank_job(candidate, job)) for job in rows if is_eligible(candidate, job)]
    ranked.sort(key=lambda item: (-item[1]["match_score"], item[0].id))
    total = len(ranked)
    selected = ranked[(page - 1) * page_size : page * page_size]
    return _page([_response(job, scores) for job, scores in selected], page, page_size, total)
